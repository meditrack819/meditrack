...
console.log("🔍 Using ML API:", ML_API); // Debug check
...
export default function Stock(){
  ...
  async function refreshDatasetInfo(){
    try{
      const { data } = await axios.get(`${API}/api/stock/analytics/dataset`);
      setDsInfo(data);
    }catch{
      setDsInfo(null);
    }
  ...
  async function handleUpload(e){
    ...
    try{
      ...
      fd.append("file", file);
      await axios.post(`${API}/api/stock/analytics/dataset`, fd, { headers: { "Content-Type": "multipart/form-data" }});
  ...
  const loadCharts = async () => {
    ...
    let hadError = false;
    try{
      const { data: f } = await axios.get(`${ML_API}/forecast_db`, { params: { horizon, include_all_stock: true } });
    ...
    try{
      ...
      const { data: r } = await axios.post(`${ML_API}/forecast_all`, payload);
  ...
  // 🔄 Updated: forecastMonths + toggle
  const forecastMonths = useMemo(() => {
    return [...new Set(forecastRows.map(r => r.forecast_month))].sort();
  }, [forecastRows]);

  const currentMonth = forecastMonths[selectedMonthIndex] || null;

  const topForecast = useMemo(() => {
    if(!currentMonth) return [];
    return forecastRows
      .filter(r => r.forecast_month === currentMonth)
      .sort((a,b)=>(b.forecast_qty||0)-(a.forecast_qty||0))
      .slice(0,10)
      .map(r => ({ name:r.medicine, value:Number(r.forecast_qty)||0 }));
  }, [forecastRows,currentMonth]);
  ...
  return (
    <div className={theme === "dark" ? "page dark" : "page"}>
      ...
      <div className="charts">
        <Panel title={`Top Forecast – ${fmtMonthName(currentMonth)}`}>
          ...
          <div style={{marginTop:8,display:"flex",justifyContent:"center",gap:8}}>
            <button className="btn" onClick={()=>setSelectedMonthIndex(i=>Math.max(0,i-1))} disabled={selectedMonthIndex===0}>◀ Prev</button>
            <span>{selectedMonthIndex+1}/{forecastMonths.length}</span>
            <button className="btn" onClick={()=>setSelectedMonthIndex(i=>Math.min(forecastMonths.length-1,i+1))} disabled={selectedMonthIndex===forecastMonths.length-1}>Next ▶</button>
          </div>
        </Panel>
