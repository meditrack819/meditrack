...
router.get('/', async (req, res) => {
  try {
    console.log("📦 [GET /stock] Fetching current inventory...");
    const result = await pool.query(
      `SELECT id, medicine_name, quantity, expiration_date, last_updated
         FROM stock_inventory
        ORDER BY id ASC`
    );
    console.log("📦 [GET /stock] Rows returned:", result.rows.length);
    return res.json(result.rows);
  } catch (err) {
    console.error('❌ GET /stock error:', err);
    res.status(500).json({ error: 'Failed to fetch stock' });
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    console.log('📝 PUT /stock/:id - Request body:', { id, medicine_name, quantity, expiration_date });
    ...
    // Store old quantity for change calculation
    const oldQty = Number(ex[0].quantity) || 0;
    console.log('📊 Old quantity:', oldQty, 'New quantity:', quantity);
    ...
    // ✅ Log quantity change in stock_movements
    if (quantity !== undefined) {
      const newQty = Number(quantity);
      const qtyChange = newQty - oldQty;
      
      console.log('🔢 Quantity change calculation:', { oldQty, newQty, qtyChange });
      
      if (qtyChange !== 0) {
        const reason = qtyChange > 0 ? 'Manual adjustment' : 'Manual reduction';
        console.log('💾 Inserting stock movement:', { 
          stock_id: id, 
          medicine_name: updated[0].medicine_name, 
        ...
        await client.query(
          `INSERT INTO stock_movements 
           (stock_id, medicine_name, change_qty, reason, created_at)
           VALUES ($1, $2, $3, $4, NOW())`,
          [
            id,
            updated[0].medicine_name,
            qtyChange,
            reason
          ]
        );
        console.log(`✅ Stock movement logged: ${updated[0].medicine_name} ${qtyChange > 0 ? '+' : ''}${qtyChange}`);
      } else {
        console.log('⚠️ No quantity change detected, skipping stock movement log');
      }
    } else {
      console.log('⚠️ Quantity was not updated, skipping stock movement log');
    }
  ...
  try {
    const result = await pool.query(`
      ...
      LEFT JOIN LATERAL (
        ...
        WHERE pr.medication_name = sm.medicine_name
          AND sm.reason ILIKE '%prescription%'
          AND pr.created_at BETWEEN (sm.created_at - INTERVAL '10 seconds') 
                                AND (sm.created_at + INTERVAL '10 seconds')
        ORDER BY ABS(EXTRACT(EPOCH FROM (pr.created_at - sm.created_at)))
        LIMIT 1
      ) matched_prescription ON true
      LEFT JOIN public.patients p 
        ON matched_prescription.patient_id = p.user_id
      ORDER BY sm.id DESC
      LIMIT 200;
    `);
    ...
    res.status(500).json({ error: 'Failed to load stock movements', detail: err.message });
  }
});
...
router.get('/analytics/ping', async (_req, res) => {
  try {
    const { data } = await axios.get(`${ML_BASE}/health`, { timeout: 5000 });
    res.json(data);
  } catch (err) {
    console.error('ML health error:', err.message);
    res.status(500).json({ error: 'ml_health_failed' });
  }
});
...
router.get('/analytics/forecast', async (req, res) => {
  ...
  try {
    const { data } = await axios.get(`${ML_BASE}/forecast_db`, { params: { horizon }, timeout: 20000 });
    res.json(data);
  } catch (err) {
    console.error('ML forecast error:', err.message);
    res.status(500).json({ error: 'ml_forecast_failed' });
  }
});
...
router.get('/analytics/seasonality', async (_req, res) => {
  try {
    const { data } = await axios.get(`${ML_BASE}/seasonality_db`, { timeout: 20000 });
    res.json(data);
  } catch (err) {
    console.error('ML seasonality error:', err.message);
    res.status(500).json({ error: 'ml_seasonality_failed' });
  }
});
...
router.get('/analytics/restock', async (req, res) => {
  const horizon = Number(req.query?.horizon ?? 6);
  try {
    const { data } = await axios.get(`${ML_BASE}/restock_db`, { params: { horizon }, timeout: 20000 });
    res.json(data);
  } catch (err) {
    console.error('ML restock error:', err.message);
